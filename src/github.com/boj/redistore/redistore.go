...
func NewRediStore(keyPairs ...[]byte) (*RediStore, error) {
	...
	//			},
	//		}, keyPairs...)
	return NewRediStoreWithPool(keyPairs...)
...
//}
//
func NewRediStoreWithPool(keyPairs ...[]byte) (*RediStore, error) {
	rs := &RediStore{
...
func (s *RediStore) Delete(r *http.Request, w http.ResponseWriter, session *sessions.Session) error {
	...
	if err := gossdb.C().Del(s.keyPrefix + session.ID); err != nil {
		return err
	}
...
func (s *RediStore) save(session *sessions.Session) error {
	...
	return gossdb.C().Set(s.keyPrefix+session.ID, b, int64(age))
}
...
func (s *RediStore) load(session *sessions.Session) (bool, error) {
	...
	data, err := gossdb.C().Get(s.keyPrefix + session.ID)
	//data, err := conn.Do("GET", s.keyPrefix+session.ID)
	if err != nil {
		return false, err
	}
