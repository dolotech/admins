...
func (this *Client) Setnx(key string, val interface{}) (Value, error) {
	...
	if err != nil {
		return Value{}, goerr.NewError(err, "Setnx %s error", key)
	}
	if len(resp) > 0 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		return Value(resp[1]), nil
	}
	return Value{}, makeError(resp, key)
}
...
func (this *Client) Getset(key string, val interface{}) (Value, error) {
	...
	if err != nil {
		return Value{}, goerr.NewError(err, "Getset %s error", key)
	}
	if len(resp) == 2 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		return Value(resp[1]), nil
	}
...
//  返回 val，字符串的部分
//  返回 err，可能的错误，操作成功返回 nil
func (this *Client) Substr(key string, start int64, size ...int64) (val Value, err error) {
	var resp []Value
	if len(size) > 0 {
	...
	if err != nil {
		return Value{}, goerr.NewError(err, "Substr %s error", key)
	}
...
//  返回 返回包含 key 的数组.
//  返回 err，可能的错误，操作成功返回 nil
func (this *Client) Keys(keyStart, keyEnd string, limit int64) ([]Value, error) {
...
//  返回 返回包含 key 的数组.
//  返回 err，可能的错误，操作成功返回 nil
func (this *Client) Rkeys(keyStart, keyEnd string, limit int64) ([]Value, error) {
