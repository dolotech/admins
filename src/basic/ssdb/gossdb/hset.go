...
func (this *Client) Hget(setName, key string) (value Value, err error) {
	...
	if err != nil {
		return Value{}, goerr.NewError(err, "Hget %s/%s error", setName, key)
	}
	if len(resp) == 2 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		return Value(resp[1]), nil
	}
	return Value{}, makeError(resp, setName, key)
}
...
func (this *Client) Hscan(setName string, keyStart, keyEnd string, limit int64) (map[string]Value, error) {
	...
	if len(resp) > 0 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		...
		for i := 1; i < size-1; i += 2 {
			re[string(resp[i])] = Value(resp[i+1])
		}
		return re, nil
	}
...
func (this *Client) MultiHget(setName string, key ...string) (val map[string]Value, err error) {
	...
	if size > 0 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		...
		for i := 1; i < size && i+1 < size; i += 2 {
			val[string(resp[i])] = Value(resp[i+1])
		}
		return val, nil
	}
...
//  返回 包含名字的数组
//  返回 err，执行的错误，操作成功返回 nil
func (this *Client) Hkeys(setName, keyStart, keyEnd string, limit int64) ([]Value, error) {
...
// 将一个go对象（可以是对象的指针 ）存储到数据库的hash表中,go对象的每个属性和属性值分别作为hash表的字段名和字段值
//  返回 err，执行的错误，操作成功返回 nil
func (this *Client) PutObject(setName, value interface{}) (err error) {
	if value == nil {
	...
	for i := 0; i < t.NumField(); i++ {
		args = append(args, t.Field(i).Name)
		args = append(args, this.encoding(v.Field(i).Interface(), false))
	}
	resp, err := this.Do("multi_hset", setName, args)
...
// 从数据库的hash表中取出整个hash表映射到go对象， value参数必须是指针
//  返回 err，执行的错误，操作成功返回 nil
func (this *Client) GetObject(setName string, value interface{}) error {
	...
	if size > 1 && string(resp[0]) == "ok" {
		for i := 1; i < size && i+1 < size; i += 2 {
			...
			if val != nil && fieldValue.Type() == reflect.ValueOf(val).Type() {
