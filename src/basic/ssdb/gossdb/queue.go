...
func (this *Client) Qpop(name string, reverse ...bool) (v Value, err error) {
	...
	if err != nil {
		return Value{}, goerr.NewError(err, "%s %s error", qpop_cmd[index], name)
	}
	if len(resp) == 2 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		return Value(resp[1]), nil
	}
	return Value{}, makeError(resp, name)
}
...
func (this *Client) QpopArray(name string, size int64, reverse ...bool) (v []Value, err error) {
	...
	respsize := len(resp)
	if respsize > 1 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
...
//  返回 v，返回元素的数组，为空时返回 nil
//  返回 err，执行的错误，操作成功返回 nil
func (this *Client) Qlist(nameStart, nameEnd string, limit int64) ([]Value, error) {
...
//  返回 v，返回元素的数组，为空时返回 nil
//  返回 err，执行的错误，操作成功返回 nil
func (this *Client) Qrlist(nameStart, nameEnd string, limit int64) ([]Value, error) {
...
func (this *Client) Qget(key string, index int64) (Value, error) {
	...
	if err != nil {
		return Value{}, goerr.NewError(err, "Qget %s error", key)
	}
	if len(resp) == 2 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		return Value(resp[1]), nil
	}
...
func (this *Client) Qfront(key string) (Value, error) {
	...
	if err != nil {
		return Value{}, goerr.NewError(err, "Qfront %s error", key)
	}
	if len(resp) == 2 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		return Value(resp[1]), nil
	}
...
func (this *Client) Qback(key string) (Value, error) {
	...
	if err != nil {
		return Value{}, goerr.NewError(err, "Qback %s error", key)
	}
	if len(resp) == 2 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		return Value(resp[1]), nil
	}
