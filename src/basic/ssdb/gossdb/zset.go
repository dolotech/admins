...
func (this *Client) MultiZget(setName string, key ...string) (val map[string]int64, err error) {
	...
	if size > 0 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		...
		for i := 1; i < size && i+1 < size; i += 2 {
			val[string(resp[i])] = Value(resp[i+1]).Int64()
		}
		return val, nil
	}
...
//  返回 val 排名
//  返回 err，可能的错误，操作成功返回 nil
//func (this *Client) Zrrange(setName string, offset, limit int64) (map[string]int64, error) {
func (this *Client) Zrrange(setName string, offset, limit int64) (map[int64]string, error) {
	...
	if len(resp) > 0 && resp[0][0] == ok[0] && resp[0][1] == ok[1] {
		...
		for i := 1; i < size-1; i += 2 {
			//glog.Infoln(Value(resp[i+1]).Int64())
			val[(Value(resp[i+1]).Int64())] = string(resp[i])
		}
		return val, nil
	}
	return nil, makeError(resp, setName, offset, limit)
...
//  返回 err，可能的错误，操作成功返回 nil
//func (this *Client) Zrrange(setName string, offset, limit int64) (map[string]int64, error) {
func (this *Client) Zrrange2(setName string, offset, limit int64) (map[string]int64, error) {
	resp, err := this.Do("zrrange", setName, this.encoding(offset), this.encoding(limit))
