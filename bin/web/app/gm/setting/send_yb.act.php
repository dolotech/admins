<?php
class Act_Send_yb extends Page{     //记得改这里
	...
	public function process() {
		...
		if (($this->input ['data'] ['yb_num']) 
				&& ($this->input ['data'] ['level_start']) 
				&& ($this->input ['data'] ['level_end'])) {
			$msg = "gmrewardbat|$this->serverid|{\"ext\":{\"{$data['data_order_field1']}\":\"{$data['data_yb_num']}\"}}"; // 按照协议格式发送消息给服务器
			...
			$url = Admin::url ( 'send_yb', 'setting'); // 指定页面的地址
			//echo $url;
	...
	private function getKeyword()
	{
		...
		if (($this->input['data']['yb_num']))
		{
			$data['data_yb_num'] = $this->input['data']['yb_num'];
		}
