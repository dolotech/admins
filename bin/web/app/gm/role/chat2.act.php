...
class Act_Chat2 extends Page
{
    ...
    private function getList($sql)
    {
        ...
        while ($row = $this->dbh->fetch_array($rs)) {
            if($row['server_id'] > 0){
                $row['args'] = '{"id":"'.$row['player_id'].'","platformid":"'.$this->platformid.'","serverid":"'.$row['server_id'].'"}'; 
                $row['level'] = Game::getLevel($row['player_id'], $row['server_id'], $this->platformid);
