<?php
class Act_Killdragon extends Page{   
	private
	...
	public function process(){
		...
		$sql = "select * from {$this->dbh->dbname}.log_killdragon1";
	...
	private function getSqlWhere($kw)
	{
		...
		//账号名
  		...
  		if (isset($kw['kw_player_name']) && strlen($kw['kw_player_name']) && ($id!=''))
 		{
        	...
        	$url = Admin::url ('log','killdragon','',true);
            
        echo "<meta http-equiv='Content-Type'' content='text/html; charset=utf-8'>";
        echo "<script>alert('名字错误');window.location.href='?mod=log&act=killdragon';</script>";
       //echo $url;
        exit;    
        }       
