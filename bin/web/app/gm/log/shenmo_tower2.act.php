<?php
class Act_Shenmo_tower2 extends Page{
	...
	public function process(){
		...
		$sql = "select * from {$this->dbh->dbname}.log_shenmo_tower2";
	...
	private function getKeyword()
	{
		...
		if (($this->input['kw']['player_name']))
        {
 		...
 		if (($this->input['kw']['mapid']))
 		{
 			$kw['kw_mapid'] = $this->input['kw']['mapid'];
 		}
	...
	{	
       ...
       //dump($id); 
 		if (isset($kw['kw_mapid']) && strlen($kw['kw_mapid']))
 		{
 			$sqlWhere .= " and mapid like('%{$kw['kw_mapid']}%')";
 		}
        ...
        echo "<script>alert('名字错误');window.location.href='';</script>";
       //echo $url;
        exit;    
        }       
